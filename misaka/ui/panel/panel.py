...
def session_arg(argv):
    ...
    pane exists, long before the session inside reports itself (core/panel/agent_state.py
...
def open_session_panes(listing):
    ...
    (core/panel/agent_state.py), or failing that the one it was launched on. One session,
    one tab."""
    live = {}
    for pane in listing:
        if not pane.get("alive"):
