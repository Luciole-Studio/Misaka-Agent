"""FTS/LIKE 查询构建（hermes-lcm search_query.py 逐字整搬，MIT；见 third_party/PINS.md）。

关键语义：CJK/emoji 查询判定走 LIKE 通道（FTS unicode61 不切 CJK 词）；
NFC 规范化；裸 AND/OR/NOT/NEAR 中性化；directness 评分反堆砌。零依赖纯函数。"""
...
if __name__ == "__main__":
    # 移植自检：钉住 misaka 最依赖的三条语义（CJK→LIKE、算子中性化、term 抽取）
    assert requires_like_fallback("入藏簿 1954", sanitize_fts5_query("入藏簿 1954")), \
        "CJK 查询必须走 LIKE 通道"
    assert not requires_like_fallback("archive 1954", sanitize_fts5_query("archive 1954"))
    assert "or" in sanitize_fts5_query("Portland OR hotel").split(), \
        "裸 OR 必须小写中性化（否则静默变析取）"
    assert extract_search_terms('"入藏簿" 档案') == ["入藏簿", "档案"]
    terms = extract_search_terms("api:v2 test")
    assert "api:v2" in terms and "api" in terms and "v2" in terms, "连字 token 双保留"
    assert compute_directness_score("入藏簿显示 1954 年", ["入藏簿", "1954"], []) > 0
    assert escape_like("50%_a\\b") == "50\\%\\_a\\\\b"
    assert compute_search_candidate_cap(10) == 500 and compute_search_candidate_cap(1000) == 5000
    print("lcm search_query selfcheck ok — CJK 通道/算子中性化/term 抽取/评分 全对")
