...
from misaka.core.web.browser.vault.backends import unlock as _unlock
from misaka.core.web.browser.vault.backends.base import (
    LoginBackend,
    UnlockRequired,
    run_with_stdin_secret,
)
from misaka.core.web.browser.vault.host import (
    _OP_ENV_ALLOWLIST,
    _scrub,
    find_op,
    run_cli,
...
class OnePasswordLoginBackend(LoginBackend):
    ...
    def __init__(self, cfg: dict | None = None):
        self.cfg = cfg or {}
        from misaka.core.web.browser.vault.host import get_secret
    ...
    def resolve_otp(self, handle: str) -> str | None:
        ...
        except (RuntimeError, UnlockRequired):
            return None
        return code if code.isdigit() else None


def _first_origin(urls: list[str]) -> str | None:
    for u in urls:
        try:
            return normalize_origin(u)
        except (VaultError, ValueError):
            pass  # Skip non-Web manager URLs without logging credential metadata.
    return None
