...
def classify_login_control(control: LoginControl) -> ClassifiedLoginControl | None:
...
def classify_checkout_control(control: LoginControl) -> ClassifiedLoginControl | None:
...
def build_otp_fills(otp_controls: list[ClassifiedLoginControl], code: str) -> list[dict[str, Any]]:
    ...
    if (len(boxes) == len(code)
            ...
            and all(b.control.index - a.control.index == 1 for a, b in pairwise(boxes))):
