...
question of what address it reaches belongs to :mod:`misaka.core.web.bounded`.
...
def allow_private_urls() -> bool:
    ...
    The ``allow_private_urls`` key of the ``web`` section (a role may set its own). Hermes also
    honours an environment variable here; MISAKA does not: its own switches are settings, and a
    process that must differ runs against its own home.

    Deliberately not cached, unlike Hermes, which memoises for the process lifetime.
    Every other read of the web configuration in MISAKA is uncached and takes effect on the next
