...
no ``security`` section and no YAML config at all; web behaviour lives in one JSON
document, ``~/.misaka/web.json`` (``CFG["web_config"]``, so tests can redirect it), and
the policy is read from a top-level ``website_blocklist`` key there::
...
(:mod:`misaka.core.tools._web.screening`, above every backend and over its own direct
...
def load_website_blocklist(config_path: Path | None = None) -> dict[str, Any]:
    ...
    # part of the test because ``CFG["web_config"]`` is redirectable at runtime.
