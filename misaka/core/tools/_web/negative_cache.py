...
redundant request -- so it is a plain dict with no persistence, no file, no lock.
...
Keys are the exact URL string the caller hands in, so the check and the record must
use the same one. A caller that follows redirects should key on the URL it asked for
rather than the one it landed on -- the asked-for URL is what the model will pick
again.

Concurrency: MISAKA drives tools from one asyncio event loop, so no lock. Dict
get/set are atomic anyway, and a failure that raced past an active ban is dropped
rather than allowed to re-time it (see ``record_failure``), so the worst an
interleaving can cost is one extra fetch.
"""
...
def record_failure(url: str, status: int, now: float | None = None) -> None:
    ...
    if entry is None or entry.status != status:
        ...
        if entry is None and len(_BANS) >= _MAX_ENTRIES:
            _evict(t)
        entry = _BANS[url] = _Ban(status)
    entry.fails += 1
...
def _evict(now: float) -> None:
    ...
    for url in [u for u, e in _BANS.items() if e.until <= now]:
        del _BANS[url]
    if len(_BANS) >= _MAX_ENTRIES:
        del _BANS[min(_BANS, key=lambda u: _BANS[u].until)]
