# Hermes f03ed94a34f47ebca57e4a1b0a890bc2aeb5e140 / agent/learn_prompt.py; see PROVENANCE.json and LICENSE.
...
Follow the Hermes skill-authoring standards with MISAKA's description budget.
These are the rules enforced in review:
...
- description: ONE sentence, **<={SKILL_PROMPT_DESC_LIMIT} characters**, ends with a period. State the
  ...
  down before saving — do not ship a sentence and hope.
    Good (<={SKILL_PROMPT_DESC_LIMIT}): `Search arXiv papers by keyword, author, or ID.`
    Bad (marketing):   `A comprehensive skill that lets the agent search arXiv for
