# Hermes f03ed94a34f47ebca57e4a1b0a890bc2aeb5e140 / tools/skills_hub_clawhub.py; see PROVENANCE.json and LICENSE.
...
class ClawHubSource(GuardedFetchMixin, SkillSource):
    ...
    def enrich_owners(self, skills: List[SkillMeta], max_workers: int = 30) -> int:
        ...
        from ..distribution_host import ThreadPoolExecutor
    ...
    def _download_zip(self, slug: str, version: str, owner: Optional[str] = None) -> Dict[str, str]:
        ...
        for attempt in range(max_retries):
            try:
                resp = http_get(f"{self.BASE_URL}/download", params=params,
                ...
                with zipfile.ZipFile(io.BytesIO(resp.content)) as zf:
                    if len(zf.infolist()) > 4096 or sum(i.file_size for i in zf.infolist()) > 64 * 1024 * 1024:
                        raise ValueError("ClawHub archive exceeds extraction budget")
