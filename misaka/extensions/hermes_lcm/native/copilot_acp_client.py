...
from ..host.acp_environment import get_read_block_error, get_write_denied_error, is_write_approval_required
from .redact import redact_sensitive_text
from ..host.acp_environment import hermes_subprocess_env
...
_PROMPT_PREAMBLE = (
    "You are being used as the active ACP agent backend for MISAKA.",
...
_INITIALIZE_PARAMS = {
    ...
    "clientInfo": {"name": "misaka-lcm", "title": "MISAKA LCM", "version": "0.0.0"},
}
_DEPRECATED_CLI_ERROR = (
    "MISAKA ACP mode requires the NEW GitHub Copilot CLI (github.com/github/copilot-cli), but the binary it just "
    "spawned is the deprecated `gh copilot` extension.\n\n"
    "Install the new CLI:\n  npm install -g @github/copilot\n  # then verify with: copilot --help\n\n"
    "If `copilot` already resolves to the new CLI but you still see this,\npoint MISAKA at it explicitly:\n"
    "  export HERMES_COPILOT_ACP_COMMAND=/path/to/new/copilot\n\n"
    "Alternative: use the `copilot` provider (no ACP, hits the Copilot API\ndirectly with a Copilot subscription "
    "token) in MISAKA settings.\n\nOriginal error:\n"
)
...
def _acp_supported(command: str, args: list[str]) -> bool | None:
    ...
    try:
        probe = subprocess.run(
            ...
            stdin=subprocess.DEVNULL, env=_build_subprocess_env(),
        )
...
class CopilotACPClient:
    ...
    def _spawn(self) -> subprocess.Popen[str]:
        ...
        try:
            from ..host.acp_environment import windows_hide_flags  # hide the Windows console flash (#56747); pipes intact for the ACP wire
    ...
    def _run_prompt(self, prompt_text: str, *, timeout_seconds: float, model: str | None = None) -> tuple[str, str]:
        ...
        pumps = [threading.Thread(target=_pump, args=(proc.stdout, lambda line: inbox.put(_decode(line))), daemon=True),
        ...
        def _request(method: str, params: dict[str, Any], *, text_parts: list[str] | None = None, reasoning_parts: list[str] | None = None) -> Any:
            ...
            while time.monotonic() < deadline:
                try:
                    ...
                    if proc.poll() is not None and not pumps[0].is_alive():
                        break
                    continue
    ...
    def _handle_server_message(
        ...
        if method == "session/request_permission":
            ...
            response = _jsonrpc_error(message_id, -32601, f"ACP client method '{method}' is not supported by MISAKA yet.")
