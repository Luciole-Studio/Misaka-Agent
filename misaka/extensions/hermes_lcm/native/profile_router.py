...
from .provider_profile import ProviderProfile, _profile_user_agent
...
def _resolve_api_key() -> str:
    ...
    try:
        from ..host.config_bridge import _scoped_key_env as prefer_dotenv
    except Exception:
        prefer_dotenv = None
...
def _disk_path() -> Optional[Path]:
    try:
        ...
        return Path(get_agent_dir()) / "cache" / "router_catalog.json"
    except Exception:
        return None
...
def _fetch_catalog_items(*, api_key: str = "", base_url: str = "", timeout: float = 8.0) -> Optional[list]:
    ...
    from ..host.profiles import open_credentialed_url
