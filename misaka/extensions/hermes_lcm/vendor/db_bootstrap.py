    ...
    with _integrity_scan_lock:
        ...
        try:
            ...
            try:
                ...
                # misaka: U06 another process may win after the optimistic read.
                started = _load_scan_started_at(claim_conn, spec)
                if started is not None and (current - started) < INTEGRITY_SCAN_STALE_SECONDS:
                    return True
...
def _fts_needs_rebuild(
    ...
    *,
    ...
    if result["status"] == "pass":
        ...
        _clear_integrity_failed(conn, spec)  # misaka: U05 only a verified pass resolves a flag
    ...
    *,
    ...
    if not fts_structure_needs_rebuild or not throttle:
        ...
        if not deep_repair_needed:
            ...
            if not _fts_missing_triggers(conn, spec):
                # misaka: U05 throttled/async/unchecked is not proof of integrity.
                conn.commit()
                return {
                    "rebuilt": False,
                    "degraded": False,
    ...
    with _fts_repair_ownership(conn) as owns_transaction:
        ...
        if rebuilt:
            ...
            # interval.
            _record_integrity_checked(conn, spec, now=now)
        # misaka: U05 trigger-only repair does not resolve indexed-token drift.
        # Clear in the transaction that actually rebuilt the index; synchronous
        # verified passes clear at their check site, never on a throttled return.
        if rebuilt:
