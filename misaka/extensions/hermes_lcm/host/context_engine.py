...
from . import config_bridge, execution, fence, ingest, llm, rollups
...
# The registry lock never spans engine work or a provider request. Weak locks live
# as long as an owner/waiter holds them, so retired session IDs do not leak locks.
...
def _key(ctx=None):
    return config_bridge.database_path(), ingest.session_id(ctx) if ctx is not None else ""
...
def engine(ctx=None):
    ...
    with operation(ctx):
        ...
        if key not in _ENGINES:
            ...
            _ENGINES[key] = NativeLCMEngine(config=config_bridge.load_config(), hermes_home="")
            from .native import session_ids
            _ENGINES[key]._dag.before_publish = execution.check_cancelled
            _ENGINES[key]._lifecycle._host_sessions = session_ids
        return _ENGINES[key]


def close(ctx=None) -> None:
    with operation(ctx):
        built = _ENGINES.pop(_key(ctx), None)
        if built is not None:
            built.shutdown()


def close_all() -> None:
    """Test/process teardown after owners have settled; never a UI detach hook."""
    with _REGISTRY_LOCK:
        pending = list(_ENGINES.values())
        _ENGINES.clear()
...
def _archive_map(built, transcript, *, through=None):
    if through is not None:
        ...
        transcript = Transcript(transcript.entries[:end], transcript.branch)
...
def _checkpoint_nodes(built, entry, transcript):
    ...
    for record in records:
        ...
        sources = sorted({*carried, *(source for ref in references for source in stored[ref])})
        ...
        if node is None:
            ...
            node = next((node for node in built._dag.get_session_nodes(built.current_session_id, limit=-1)
                         if node.expand_hint == hint and node.summary == record['summary']), None)
            if node is None:
...
def _summary_scope(built, *, transcript):
    ...
    try:
        ...
        from . import carry
        built.active_store_ids = {source for entry in transcript.branch for source in stored.get(entry['id'], [])} | carry.sources(built, transcript).keys()
...
def compact(prepared):
    ...
    with _summary_scope(built, transcript=prepared.transcript):
        ...
        records = []
        from . import carry
        foreign = carry.sources(built, prepared.transcript)
        for node in frontier:
            ...
            records.append({'id': node.node_id, 'depth': node.depth, 'summary': node.summary,
                            ...
                            **({'foreignSources': [source for source in sources if source in foreign]} if any(source in foreign for source in sources) else {})})
        scaffolds = []
        native_metadata = {}
        guarded = []
